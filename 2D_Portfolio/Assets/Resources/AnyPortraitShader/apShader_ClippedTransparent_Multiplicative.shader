...
Shader "AnyPortrait/Transparent/Clipped Texture (2X) Multiplicative"
{
	Properties
	{
	...
	SubShader
	{
		...
		{	
			...
			half4 frag(vertexOutput IN) : COLOR
			{
				...
				//half4 c_mask = tex2D(_MaskTex, IN.uv_mask) * _MaskColor;
				//half4 c_main = tex2D(_MainTex, IN.uv) * _Color;
		...
		Pass
		{
			...
			half4 frag(vertexOutput IN) : COLOR
			{
				...
				float3 itpColor = (c.rgb * itp) + grayColor * (1.0f - itp);
				c.rgb = itpColor;
