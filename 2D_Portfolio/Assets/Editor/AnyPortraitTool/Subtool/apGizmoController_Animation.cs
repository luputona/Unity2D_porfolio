...
namespace AnyPortrait
{
	...
	public partial class apGizmoController
	{
			...
			if (!isSelectionLock)
			{
				if (isBoneTarget && !isVertexSelected)
				{
						...
						selectType == apGizmos.SELECT_TYPE.Add)
					{
						...
						{
							bone = CheckBoneClick(boneList[i], mousePosW, mousePosGL, Editor._boneGUIRenderMode == apEditor.BONE_RENDER_MODE.RenderOutline);
							if (bone != null)
							{
								resultBone = bone;
							}
		...
		public void Move__Animation_Transform(Vector2 curMouseGL, Vector2 curMousePosW, Vector2 deltaMoveW, int btnIndex, bool isFirstMove)
		{
			...
			if (isFirstMove)
			{
				apEditorUtil.SetRecord(apUndoGroupData.ACTION.Anim_Gizmo_MoveTransform, targetMeshGroup, targetRenderUnit, false, Editor);
			}
			...
			if (isTargetTransform)
			{
				...
				if (selectedTransform != _prevSelected_Transform || isFirstMove)
				{
					_prevSelected_Transform = selectedTransform;
			...
			else
			{
				...
				if (bone._isIKTail)
				{
					...
					apBone limitedHeadBone = bone.RequestIK_Limited(curMousePosW, weight, true, Editor.Select.ModRegistableBones);
		...
		public void Move__Animation_Vertex(Vector2 curMouseGL, Vector2 curMousePosW, Vector2 deltaMoveW, int btnIndex, bool isFirstMove)
		{
			...
			if (isFirstMove)
			{
				apEditorUtil.SetRecord(apUndoGroupData.ACTION.Anim_Gizmo_MoveVertex, linkedModifier._meshGroup, targetVertex, isMultipleVerts, Editor);
			}
		...
		public void Rotate__Animation_Transform(float deltaAngleW)
		{
			if (Editor.Select.AnimClip == null ||
				Editor.Select.AnimClip._targetMeshGroup == null ||
				Editor.Select.AnimTimeline == null)
			...
			//Undo
			apEditorUtil.SetRecord(apUndoGroupData.ACTION.Anim_Gizmo_RotateTransform, targetMeshGroup, targetRenderUnit, false, Editor);
		...
		/// </summary>
		/// <param name="deltaAngleW"></param>
		public void Rotate__Animation_Vertex(float deltaAngleW)
		{
			if (Editor.Select.AnimClip == null || Editor.Select.AnimClip._targetMeshGroup == null)
			{
				return;
			...
			apEditorUtil.SetRecord(apUndoGroupData.ACTION.Anim_Gizmo_RotateVertex, linkedModifier._meshGroup, targetVert, isMultipleVerts, Editor);
		...
		public void Scale__Animation_Transform(Vector2 deltaScaleW)
		{
			if (Editor.Select.AnimClip == null ||
				Editor.Select.AnimClip._targetMeshGroup == null ||
				Editor.Select.AnimTimeline == null)
			...
			//Undo
			apEditorUtil.SetRecord(apUndoGroupData.ACTION.Anim_Gizmo_ScaleTransform, targetMeshGroup, targetRenderUnit, false, Editor);
		...
		/// </summary>
		/// <param name="deltaScaleW"></param>
		public void Scale__Animation_Vertex(Vector2 deltaScaleW)
		{
			if (Editor.Select.AnimClip == null || Editor.Select.AnimClip._targetMeshGroup == null)
			{
				return;
			...
			apEditorUtil.SetRecord(apUndoGroupData.ACTION.Anim_Gizmo_ScaleVertex, linkedModifier._meshGroup, targetVert, isMultipleVerts, Editor);
		...
		public void TransformChanged_Position__Animation_Vertex(Vector2 pos, int depth)
		{
			...
			apEditorUtil.SetRecord(apUndoGroupData.ACTION.Anim_Gizmo_MoveVertex, linkedModifier._meshGroup, targetVert, isMultipleVerts, Editor);
		...
		public void TransformChanged_Rotate__Animation_Transform(float angle)
		{
			...
			else if (isTargetBone)
			{
				...
				{
					if (angle < bone._IKAngleRange_Lower)
					{
						angle = bone._IKAngleRange_Lower;
					}
					else if (angle > bone._IKAngleRange_Upper)
					{
						angle = bone._IKAngleRange_Upper;
					}
				}
		...
		public void TransformChanged_Color__Animation_Transform(Color color, bool isVisible)
		{
			...
			//Undo
			apEditorUtil.SetRecord(apUndoGroupData.ACTION.Anim_Gizmo_Color, Editor.Select.AnimClip._targetMeshGroup, targetRenderUnit, false, Editor);
		...
		public bool FFDTransform__Animation_Vertex(List<object> srcObjects, List<Vector2> posWorlds, bool isResultAssign)
		{
			...
			//Undo
			apEditorUtil.SetRecord(apUndoGroupData.ACTION.Anim_Gizmo_FFDVertex, targetMeshGroup, null, true, Editor);
		...
		public bool PressBlur_Animation_Vertex(Vector2 pos, float tDelta)
		{
			if (Editor.Select.AnimClip == null || Editor.Select.AnimClip._targetMeshGroup == null)
			{
				return false;
			...
			if (Editor.Select.ModRenderVertListOfAnim == null || Editor.Select.ModRenderVertListOfAnim.Count <= 1)
			{
				return false;
			}
