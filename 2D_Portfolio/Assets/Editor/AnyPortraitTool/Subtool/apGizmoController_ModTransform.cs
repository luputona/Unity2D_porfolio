...
namespace AnyPortrait
{
	...
	public partial class apGizmoController
	{
		...
		private object _prevSelected_Transform = null;
		...
		public void Move__Modifier_Transform(Vector2 curMouseGL, Vector2 curMousePosW, Vector2 deltaMoveW, int btnIndex, bool isFirstMove)
		{
			...
			if (isFirstMove)
			{
				apEditorUtil.SetRecord(apUndoGroupData.ACTION.Modifier_Gizmo_MoveTransform, Editor.Select.Modifier._meshGroup, targetObj, false, Editor);
			}
			...
			if (isTargetTransform)
			{
				...
				if (Editor.Select.ExValue_ModMesh._isMeshTransform)
				{
					if (Editor.Select.ExValue_ModMesh._transform_Mesh != null)
					{
						...
						resultMatrixWithoutMod = targetModMesh._transform_Mesh._matrix_TFResult_WorldWithoutMod;
				...
				if (localStaticMatrix == null || resultMatrix == null)
				{
					...
					//Editor.Select.ExValue_ModMesh._transformMatrix.SetPos(nextPos);
				...
				Profiler.BeginSample("Gizmo - ModTransform Move [Refresh]");//>>>>>

				targetModMesh.RefreshModifiedValues(Editor._portrait);


				//강제로 업데이트할 객체를 선택하고 Refresh
				//Editor.Select.MeshGroup.AddForceUpdateTarget(targetModMesh._renderUnit);
		...
		public void Rotate__Modifier_Transform(float deltaAngleW)
		{
			if (Editor.Select.MeshGroup == null ||
				Editor.Select.Modifier == null ||
				deltaAngleW == 0.0f)
			...
			{ targetObj = Editor.Select.ModBoneOfMod; }
			apEditorUtil.SetRecord(apUndoGroupData.ACTION.Modifier_Gizmo_RotateTransform, Editor.Select.Modifier._meshGroup, targetObj, false, Editor);
		...
		public void Scale__Modifier_Transform(Vector2 deltaScaleW)
		{
			if (Editor.Select.MeshGroup == null ||
				Editor.Select.Modifier == null ||
			...
			{ targetObj = Editor.Select.ModBoneOfMod; }
			apEditorUtil.SetRecord(apUndoGroupData.ACTION.Modifier_Gizmo_ScaleTransform, Editor.Select.Modifier._meshGroup, targetObj, false, Editor);
		...
		public void TransformChanged_Color__Modifier_Transform(Color color, bool isVisible)
		{
			...
			//Undo
			apEditorUtil.SetRecord(apUndoGroupData.ACTION.Modifier_Gizmo_Color, Editor.Select.Modifier._meshGroup, Editor.Select.ExValue_ModMesh, false, Editor);
