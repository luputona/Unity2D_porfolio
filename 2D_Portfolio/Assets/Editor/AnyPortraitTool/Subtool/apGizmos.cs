...
namespace AnyPortrait
{
	...
	public class apGizmos
	{
		...
		public void IncreaseSoftSelectionRadius() { if (_isSoftSelectionMode) { _softSelectionRadius = Mathf.Clamp(_softSelectionRadius + 10, 0, MAX_SOFT_SELECTION_RADIUS); RefreshSoftSelectionValue(_softSelectionRadius, _softSelectionCurveRatio); } }
		public void DecreaseSoftSelectionRadius() { if (_isSoftSelectionMode) { _softSelectionRadius = Mathf.Clamp(_softSelectionRadius - 10, 0, MAX_SOFT_SELECTION_RADIUS); RefreshSoftSelectionValue(_softSelectionRadius, _softSelectionCurveRatio); } }
		...
		public void IncreaseBlurRadius() { if (_isBlurMode) { _blurRadius = Mathf.Clamp(_blurRadius + 10, 0, MAX_BLUR_RADIUS); RefreshBlurValue(_blurRadius, _blurIntensity); } }
		public void DecreaseBlurRadius() { if (_isBlurMode) { _blurRadius = Mathf.Clamp(_blurRadius - 10, 0, MAX_BLUR_RADIUS); RefreshBlurValue(_blurRadius, _blurIntensity); } }
		...
		public delegate void FUNC_GIZMO_EVENT__ROTATE(float deltaAngleW);
		public delegate void FUNC_GIZMO_EVENT__SCALE(Vector2 deltaScaleW);
		...
		//Blur 이벤트
		//마우스를 우클릭했을 때의 이벤트.
		public delegate bool FUNC_GIZMO_EVENT__PRESS_BLUR(Vector2 pos, float tDelta);
		...
		public Texture2D GetImage(IMAGE_TYPE gizmosImageType)
		{
			...
			{
				case IMAGE_TYPE.Origin_None:
					return Editor.ImageSet.Get(apImageSet.PRESET.Gizmo_OriginNone);
				case IMAGE_TYPE.Origin_Axis:
					return Editor.ImageSet.Get(apImageSet.PRESET.Gizmo_OriginAxis);
				case IMAGE_TYPE.Transform_Move:
					return Editor.ImageSet.Get(apImageSet.PRESET.Gizmo_Transform_Move);
				case IMAGE_TYPE.Transform_Rotate:
					return Editor.ImageSet.Get(apImageSet.PRESET.Gizmo_Transform_Rotate);
				case IMAGE_TYPE.Transform_Scale:
					return Editor.ImageSet.Get(apImageSet.PRESET.Gizmo_Transform_Scale);
				case IMAGE_TYPE.Helper:
					return Editor.ImageSet.Get(apImageSet.PRESET.Gizmo_Helper);
				case IMAGE_TYPE.Bone_Origin:
					return Editor.ImageSet.Get(apImageSet.PRESET.Gizmo_Bone_Origin);
				case IMAGE_TYPE.Bone_Body:
					return Editor.ImageSet.Get(apImageSet.PRESET.Gizmo_Bone_Body);
				case IMAGE_TYPE.TransformController:
					return Editor.ImageSet.Get(apImageSet.PRESET.TransformControlPoint);
			}
			return null;
		}
		...
		public void LoadResources()
		{
			...
			{
				//_Images.Add(IMAGE_TYPE.Origin_None,			AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/Editor/AnyPortraitTool/Images/Gizmo_Origin_None.png"));
				//_Images.Add(IMAGE_TYPE.Origin_Axis,			AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/Editor/AnyPortraitTool/Images/Gizmo_Origin_Axis.png"));

				//_Images.Add(IMAGE_TYPE.Transform_Move,		AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/Editor/AnyPortraitTool/Images/Gizmo_Transform_Move.png"));
				//_Images.Add(IMAGE_TYPE.Transform_Rotate,	AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/Editor/AnyPortraitTool/Images/Gizmo_Transform_Rotate.png"));
				//_Images.Add(IMAGE_TYPE.Transform_Scale,		AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/Editor/AnyPortraitTool/Images/Gizmo_Transform_Scale.png"));

				//_Images.Add(IMAGE_TYPE.Helper,				AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/Editor/AnyPortraitTool/Images/Gizmo_Helper.png"));

				//_Images.Add(IMAGE_TYPE.Bone_Origin,			AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/Editor/AnyPortraitTool/Images/Gizmo_Bone_Origin.png"));
				//_Images.Add(IMAGE_TYPE.Bone_Body,			AssetDatabase.LoadAssetAtPath<Texture2D>("Assets/Editor/AnyPortraitTool/Images/Gizmo_Bone_Body.png"));
		...
		public void Unlink()
		{
			if (_isFFDMode)
			{
				//만약 Transform 중이었다면
				//Cancel은 코드상 어렵겠지;
				//bool isResult = EditorUtility.DisplayDialog("Confirm Transform", "Adapt Transformation?", "Adapt", "Revert");
				bool isResult = EditorUtility.DisplayDialog(_editor.GetText(apLocalization.TEXT.AdaptFFDTransformEdit_Title),
																_editor.GetText(apLocalization.TEXT.AdaptFFDTransformEdit_Body),
																_editor.GetText(apLocalization.TEXT.AdaptFFDTransformEdit_Okay),
																_editor.GetText(apLocalization.TEXT.AdaptFFDTransformEdit_No)
																);
				if (isResult)
				{
					AdaptTransformObjects();
				}
				else
				{
					RevertTransformObjects();
				}
			}
							...
							Vector2 mousePos, bool isCtrlKey, bool isShiftKey, bool isAltKey)
		{
				...
				if (IsBlurMode)
				{
						...
						leftBtnStatus == apMouse.MouseBtnStatus.Pressed)
					{
						if (_funcGizmoPressBlur != null)
						{
							...
							_funcGizmoPressBlur(mousePos, _tMouseUpdate_Blur);
							_tMouseUpdate_Blur = 0.0f;
						}
					}
				...
				else
				{
					...
					switch (_controlType)
					{
						...
						case CONTROL_TYPE.Move:
							Update_Move(_curBtnStatus_Left, mousePos, mousePosW);
							break;

						case CONTROL_TYPE.Rotate:
							Update_Rotate(_curBtnStatus_Left, mousePos, mousePosW);
							break;

						case CONTROL_TYPE.Scale:
							Update_Scale(_curBtnStatus_Left, mousePos, mousePosW);
							break;
					}
				}
			...
			if (_curTransformParam != null)
			{
				...
				_mtrx_scale_axisY = gizmoMatrix_Local * apMatrix3x3.TRS(new Vector2(0, _axisImageSize_Scale.y / 2), 0, Vector2.one);
		...
		private void Update_Move(apMouse.MouseBtnStatus leftBtnStatus, Vector2 mousePosGL, Vector2 mousePosW)
		{
			switch (leftBtnStatus)
			{
				case apMouse.MouseBtnStatus.Down:
		...
		private void Update_Rotate(apMouse.MouseBtnStatus leftBtnStatus, Vector2 mousePosGL, Vector2 mousePosW)
		{
			switch (leftBtnStatus)
			{
				case apMouse.MouseBtnStatus.Down:
		...
		private void Update_Scale(apMouse.MouseBtnStatus leftBtnStatus, Vector2 mousePosGL, Vector2 mousePosW)
		{
			switch (leftBtnStatus)
			{
				case apMouse.MouseBtnStatus.Down:
		...
		// Trasform 모드 [FFD]
		//--------------------------------------------------------------------------
		public bool StartTransformMode(int numX = 3, int numY = 3)
		{
		...
		public void AdaptTransformObjects()
		{
			List<object> resultObject = new List<object>();
			List<Vector2> resultPos = new List<Vector2>();
			for (int i = 0; i < _transformedObjects.Count; i++)
		...
		public void RevertTransformObjects()
		{
			List<object> resultObject = new List<object>();
			List<Vector2> resultPos = new List<Vector2>();
			for (int i = 0; i < _transformedObjects.Count; i++)
