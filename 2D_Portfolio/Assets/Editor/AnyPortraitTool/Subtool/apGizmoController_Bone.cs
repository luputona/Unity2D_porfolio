...
namespace AnyPortrait
{
	...
	public partial class apGizmoController
	{
		...
		private apBone CheckBoneClick(apBone targetBone, Vector2 mousePosW, Vector2 mousePosGL, bool isDrawBoneOutline)
		{
			apBone resultBone = null;
			apBone bone = null;
			...
			{
				bone = CheckBoneClick(targetBone._childBones[i], mousePosW, mousePosGL, isDrawBoneOutline);
				if (bone != null)
				{
					resultBone = bone;
				}
			...
			if (resultBone == null)
			{
				if (IsBoneClick(targetBone, mousePosW, mousePosGL, isDrawBoneOutline))
				{
					return targetBone;
				}
			}
		...
		public bool IsBoneClick(apBone bone, Vector2 mousePosW, Vector2 mousePosGL, bool isOutline)
		{
			if (!isOutline)
			{
				//뿔 모양의 Bone Shape를 클릭하는지 체크하자
		...
		public void Rotate__Bone_Default(float deltaAngleW)
		{
			if (Editor.Select.MeshGroup == null
				|| Editor.Select.Bone == null
				|| Editor._boneGUIRenderMode != apEditor.BONE_RENDER_MODE.Render
