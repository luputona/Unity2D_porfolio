...
namespace AnyPortrait
{
	...
	public class apDialog_AddAnimTimeline : EditorWindow
	{
		...
		public void Init(apEditor editor, object loadKey, apAnimClip targetAnimGroup, FUNC_ADD_TIMELINE funcResult)
		{
			...
			AddLinkableData(apAnimClip.LINK_TYPE.ControlParam, "Control Parameters", -1);
		...
		void OnGUI()
		{
			...
			GUILayout.Space(10);
			GUILayout.Button("Select Timeline Type to Add", guiStyle_Center, GUILayout.Width(width), GUILayout.Height(15));//<투명 버튼
			GUILayout.Space(10);

			_scrollList = EditorGUILayout.BeginScrollView(_scrollList, GUILayout.Width(width), GUILayout.Height(height - (90)));

			GUILayout.Button(new GUIContent("Timeline Types", iconImageCategory), guiStyle, GUILayout.Height(20));//<투명 버튼
			...
			for (int i = 0; i < _linkDataList.Count; i++)
			{
				...
				GUILayout.Space(15);
				if (GUILayout.Button(new GUIContent(" " + _linkDataList[i]._name, _linkDataList[i]._icon), guiStyle, GUILayout.Width(width - 35), GUILayout.Height(20)))
				{
