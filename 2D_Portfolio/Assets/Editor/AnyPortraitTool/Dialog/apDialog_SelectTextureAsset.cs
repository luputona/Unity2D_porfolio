...
namespace AnyPortrait
{
	...
	public class apDialog_SelectTextureAsset : EditorWindow
	{
		...
		void OnGUI()
		{
			...
			if (GUILayout.Button("Refresh", GUILayout.Width(100), GUILayout.Height(15)))
			{
				RefreshTextureAssets();
			}
			...
			if (_curSelectedTexture2D != null)
			{
				if (_curSelectedTexture2D.name.Length < 15)
				{
			...
			if (GUILayout.Button(strSelectBtn, GUILayout.Height(30), GUILayout.Width(width / 2 - 6)))
			{
				_funcResult(true, _targetTextureData, _loadKey, _curSelectedTexture2D);
				isClose = true;
			}
			if (GUILayout.Button("Close", GUILayout.Height(30), GUILayout.Width(width / 2 - 6)))
			{
				isClose = true;
			}
			EditorGUILayout.EndHorizontal();
