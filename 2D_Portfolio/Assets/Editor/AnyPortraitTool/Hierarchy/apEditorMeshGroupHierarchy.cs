...
namespace AnyPortrait
{
	...
	public class apEditorMeshGroupHierarchy
	{
		...
		public void OnUnitVisibleClick(apEditorHierarchyUnit eventUnit, int savedKey, object savedObj, bool isVisible, bool isPrefixButton)
		{
			...
			else
			{
				...
				if (Editor._meshGroupEditMode == apEditor.MESHGROUP_EDIT_MODE.Setting)
				{
					...
					apEditorUtil.SetRecord(apUndoGroupData.ACTION.MeshGroup_DefaultSettingChanged, Editor.Select.MeshGroup, savedObj, false, Editor);
		...
		//---------------------------------------------
		//Hierarchy 레이아웃 출력
		public void GUI_RenderHierarchy(int width, bool isMeshHierarchy, float scrollX)
		{
			if (isMeshHierarchy)
			{
				...
				for (int i = 0; i < _units_Root_Meshes.Count; i++)
				{
					GUI_RenderUnit(_units_Root_Meshes[i], 0, width, scrollX);
					GUILayout.Space(10);
				}
			}
			else
			{
				...
				for (int i = 0; i < _units_Root_Bones.Count; i++)
				{
					GUI_RenderUnit(_units_Root_Bones[i], 0, width, scrollX);
					GUILayout.Space(10);
				}
			}
