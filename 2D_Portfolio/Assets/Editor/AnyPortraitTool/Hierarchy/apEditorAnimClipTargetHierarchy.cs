...
namespace AnyPortrait
{
	...
	public class apEditorAnimClipTargetHierarchy
	{
		...
		private void ReloadGUIContent()
		{
			...
			if (_guiContent_Visible_Current == null)
			{ _guiContent_Visible_Current = new GUIContent(Editor.ImageSet.Get(apImageSet.PRESET.Hierarchy_Visible_Current)); }
			if (_guiContent_NonVisible_Current == null)
			{ _guiContent_NonVisible_Current = new GUIContent(Editor.ImageSet.Get(apImageSet.PRESET.Hierarchy_NonVisible_Current)); }
			if (_guiContent_Visible_TmpWork == null)
			{ _guiContent_Visible_TmpWork = new GUIContent(Editor.ImageSet.Get(apImageSet.PRESET.Hierarchy_Visible_TmpWork)); }
			if (_guiContent_NonVisible_TmpWork == null)
			{ _guiContent_NonVisible_TmpWork = new GUIContent(Editor.ImageSet.Get(apImageSet.PRESET.Hierarchy_NonVisible_TmpWork)); }
			if (_guiContent_Visible_Default == null)
			{ _guiContent_Visible_Default = new GUIContent(Editor.ImageSet.Get(apImageSet.PRESET.Hierarchy_Visible_Default)); }
			if (_guiContent_NonVisible_Default == null)
			{ _guiContent_NonVisible_Default = new GUIContent(Editor.ImageSet.Get(apImageSet.PRESET.Hierarchy_NonVisible_Default)); }
			if (_guiContent_Visible_ModKey == null)
			{ _guiContent_Visible_ModKey = new GUIContent(Editor.ImageSet.Get(apImageSet.PRESET.Hierarchy_Visible_ModKey)); }
			if (_guiContent_NonVisible_ModKey == null)
			{ _guiContent_NonVisible_ModKey = new GUIContent(Editor.ImageSet.Get(apImageSet.PRESET.Hierarchy_NonVisible_ModKey)); }
			if (_guiContent_NoKey == null)
			{ _guiContent_NoKey = new GUIContent(Editor.ImageSet.Get(apImageSet.PRESET.Hierarchy_NoKey)); }
		}
		...
		public void ResetSubUnits()
		{
			...
			{
				_rootUnit_Bone = AddUnit_Label(null, "Bones", CATEGORY.MainName, null, null);
				//TODO..
			...
			_rootUnit_ControlParam = AddUnit_Label(null, "Control Parameters", CATEGORY.MainName, null, null);
			...
			for (int i = 0; i < controlParams.Count; i++)
			{
				...
				apEditorHierarchyUnit addedUnit = AddUnit_ToggleButton(
												//iconImage_Control,
		...
		private void AddTransformOfMeshGroup(apMeshGroup targetMeshGroup, apEditorHierarchyUnit parentUnit)
		{
			...
			for (int i = 0; i < childMeshTransforms.Count; i++)
			{
				...
				apEditorHierarchyUnit addedUnit = AddUnit_ToggleButton_Visible(iconImage,
								...
								Texture2D iconNormal, Texture2D iconIKHead, Texture2D iconIKChained, Texture2D iconIKSingle)
		{
			...
			apEditorHierarchyUnit addedUnit = AddUnit_ToggleButton(icon,
														bone._name,
														CATEGORY.Bone_Item,
														bone,
														isModRegisted,
		...
		public void OnUnitVisibleClick(apEditorHierarchyUnit eventUnit, int savedKey, object savedObj, bool isVisible, bool isPrefixButton)
		{
			...
			else
			{
				...
				if (targetTimelineLayer != null)
				{
					...
					apEditorUtil.SetRecord(apUndoGroupData.ACTION.Anim_KeyframeValueChanged, Editor.Select.AnimClip._targetMeshGroup, savedObj, false, Editor);
		...
		//---------------------------------------------
		//Hierarchy 레이아웃 출력
		public int GUI_RenderHierarchy_Transform(int width, float scrollX)
		{
			int maxLevel = 0;
			//루트 노드는 For문으로 돌리고, 그 이후부터는 재귀 호출
			for (int i = 0; i < _units_Root_Transform.Count; i++)
			{
				int curLevel = GUI_RenderUnit(_units_Root_Transform[i], 0, width, scrollX);
		...
		public int GUI_RenderHierarchy_Bone(int width, float scrollX)
		{
			int maxLevel = 0;
			//루트 노드는 For문으로 돌리고, 그 이후부터는 재귀 호출
			for (int i = 0; i < _units_Root_Bone.Count; i++)
			{
				int curLevel = GUI_RenderUnit(_units_Root_Bone[i], 0, width, scrollX);
				if (curLevel > maxLevel)
				{
					maxLevel = curLevel;
				}
		...
		public int GUI_RenderHierarchy_ControlParam(int width, float scrollX)
		{
			int maxLevel = 0;
			//루트 노드는 For문으로 돌리고, 그 이후부터는 재귀 호출
			for (int i = 0; i < _units_Root_ControlParam.Count; i++)
			{
				int curLevel = GUI_RenderUnit(_units_Root_ControlParam[i], 0, width, scrollX);
				if (curLevel > maxLevel)
				{
					maxLevel = curLevel;
				}
		...
		private int GUI_RenderUnit(apEditorHierarchyUnit unit, int level, int width, float scrollX)
		{
			int maxLevel = level;
			...
			if (unit.IsFoldOut)
			{
				if (unit._childUnits.Count > 0)
				{
					...
					{
						//재귀적으로 호출
						int curLevel = GUI_RenderUnit(unit._childUnits[i], level + 1, width, scrollX);
						if (curLevel > maxLevel)
						{
							maxLevel = curLevel;
						}
