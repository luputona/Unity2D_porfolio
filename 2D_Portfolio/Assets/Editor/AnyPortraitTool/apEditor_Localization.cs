...
namespace AnyPortrait
{
	...
	public class apEditor_Localization : EditorWindow
	{
		...
		// Show Window
		//------------------------------------------------------------------
		//[MenuItem("Window/AnyPortrait/Localization Table", false, 61)]//<<사용할때만 이걸 켭시다.
		...
		private void Save()
		{
			...
			try
			{
				...
				sw.WriteLine("Text Type,Text Index,1 English (영어),2 Korean (한국어),3 French (프랑스어),4 German (독일어),5 Spanish (스페인어),6 Italian (이탈리아어),7 Danish (덴마크어),8 Japanese (일본어),10 Chinese_Traditional (중국어-번체),11 Chinese_Simplified (중국어-간체)");
				TextSet curSet = null;
				for (int i = 0; i < _textSets.Count; i++)
				{
					curSet = _textSets[i];
					...
					strBuilder.Append(curSet.GetTextToSave(apEditor.LANGUAGE.Chinese_Simplified));                //11 : Chinese_Simplified (중국어-간체)
