...
namespace AnyPortrait
{
	...
	public static class apAnimCurveGL
	{
		...
		{
			_matBatch.SetShader(shader_Color, shader_Texture_Normal_Set, shader_Texture_VColorAdd_Set, shader_MaskedTexture_Set, shader_MaskOnly, shader_Clipped_Set);
		}
											...
											int totalEditorWidth, int totalEditorHeight)
		{
			...
			_glScreenClippingSize.w = (float)(posY + _layoutHeight) / (float)totalEditorHeight;
		...
		private static bool IsMouseInLayout(Vector2 mousePos)
		{
			if (mousePos.x < 0.0f || mousePos.x > _layoutWidth ||
				mousePos.y < 0.0f || mousePos.y > _layoutHeight)
			{
				return false;
			}
