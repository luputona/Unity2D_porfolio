...
namespace AnyPortrait
{
	...
	public static class apGL
	{
		...
		public class MaterialBatch
		{
			public enum MatType
			{
				None, Color, Texture_Normal, Texture_VColorAdd, MaskedTexture, MaskOnly, Clipped,
			}
			...
			public void SetPass_MaskedTexture(Color color, Texture2D texture,
												Color clipColor_1, Texture2D clipTexture_1,
												Color clipColor_2, Texture2D clipTexture_2,
												Color clipColor_3, Texture2D clipTexture_3,
												int pass,
												float vertexColorRatio = 0.0f
				)
			{
				_color = color;
				_mat_MaskedTexture[_shaderType_Main].SetColor("_Color", _color);

				_texture = texture;
				_mat_MaskedTexture[_shaderType_Main].SetTexture("_MainTex", _texture);

				_mat_MaskedTexture[_shaderType_Main].SetFloat("_vColorITP", vertexColorRatio);

				_clipColor_1 = clipColor_1;
				_clipColor_2 = clipColor_2;
				_clipColor_3 = clipColor_3;

				_clipTexture_1 = clipTexture_1;
				_clipTexture_2 = clipTexture_2;
				_clipTexture_3 = clipTexture_3;

				_mat_MaskedTexture[_shaderType_Main].SetColor("_Color1", _clipColor_1);
				_mat_MaskedTexture[_shaderType_Main].SetColor("_Color2", _clipColor_2);
				_mat_MaskedTexture[_shaderType_Main].SetColor("_Color3", _clipColor_3);

				_mat_MaskedTexture[_shaderType_Main].SetTexture("_ClipTexture1", _clipTexture_1);
				_mat_MaskedTexture[_shaderType_Main].SetTexture("_ClipTexture2", _clipTexture_2);
				_mat_MaskedTexture[_shaderType_Main].SetTexture("_ClipTexture3", _clipTexture_3);
				...
				{
					//RenderTexture를 끄고, 저장된 RenderTexture를 Material에 넣어준다.
					RenderTexture.active = null;
					_mat_MaskedTexture[_shaderType_Main].SetTexture("_MaskRenderTexture", _renderTexture);

					_mat_MaskedTexture[_shaderType_Main].SetColor("_BlendOpt1", _shaderTypeColor_Clip1);
					_mat_MaskedTexture[_shaderType_Main].SetColor("_BlendOpt2", _shaderTypeColor_Clip2);
					_mat_MaskedTexture[_shaderType_Main].SetColor("_BlendOpt3", _shaderTypeColor_Clip3);
				}


				_mat_MaskedTexture[_shaderType_Main].SetPass(pass);
			...
			{
				return (_mat_Color == null || _mat_Texture_Normal == null || _mat_Texture_VColorAdd == null || _mat_MaskedTexture == null || _mat_Clipped == null || _mat_MaskOnly == null);
			}
		...
		// Draw Mesh
		//-------------------------------------------------------------------------------
		public static void DrawMesh(apMesh mesh, apMatrix3x3 matrix, Color color2X, RENDER_TYPE renderType, apVertexController vertexController, apEditor editor)
		{
			try
			{
				//0. 메시, 텍스쳐가 없을 때
				...
				if (mesh._indexBuffer.Count >= 3)
				{
					...
					for (int i = 0; i < mesh._indexBuffer.Count; i += 3)
					{
						...
						Vector2 pos_0_World = vert0._pos;
						Vector2 pos_1_World = vert1._pos;
						Vector2 pos_2_World = vert2._pos;

						Vector2 pos_0 = World2GL(matrix.MultiplyPoint(pos_0_World));
						Vector2 pos_1 = World2GL(matrix.MultiplyPoint(pos_1_World));
						Vector2 pos_2 = World2GL(matrix.MultiplyPoint(pos_2_World));
						...
						switch (iVertColor)
						{
							case 1: //VolumeWeightColor
								color0 = GetWeightGrayscale(vert0._volumeWeight);
								color1 = GetWeightGrayscale(vert1._volumeWeight);
								color2 = GetWeightGrayscale(vert2._volumeWeight);
								break;

							case 2: //PhysicsWeightColor
								color0 = GetWeightGrayscale(vert0._physicsWeight);
								color1 = GetWeightGrayscale(vert1._physicsWeight);
								color2 = GetWeightGrayscale(vert2._physicsWeight);
								break;
				...
				if ((renderType & RENDER_TYPE.Vertex) != 0)
				{
					...
					for (int i = 0; i < mesh._vertexData.Count; i++)
					{
						...
						DrawBox(new Vector2(pos.x, pos.y), pointSize, pointSize, vColor, isWireFramePoint, false);
					}
			...
			try
			{
				...
				if (mesh._indexBuffer.Count >= 3 && isMeshRender && isVisible)
				{
					...
					for (int i = 0; i < mesh._indexBuffer.Count; i += 3)
					{
						...
						Vector3 pos_0 = rVert0._pos_GL;
						Vector3 pos_1 = rVert1._pos_GL;
						Vector3 pos_2 = rVert2._pos_GL;
				...
				if (isDrawTFBorderLine)
				{
					...
					if (mesh._edges.Count > 0)
					{
						for (int i = 0; i < mesh._edges.Count; i++)
						{
							...
							if (rVert0._pos_LocalOnMesh.x < minPosLocal_X)
							{ minPosLocal_X = rVert0._pos_LocalOnMesh.x; }
							if (rVert0._pos_LocalOnMesh.x > maxPosLocal_X)
							{ maxPosLocal_X = rVert0._pos_LocalOnMesh.x; }
							if (rVert0._pos_LocalOnMesh.y < minPosLocal_Y)
							{ minPosLocal_Y = rVert0._pos_LocalOnMesh.y; }
							if (rVert0._pos_LocalOnMesh.y > maxPosLocal_Y)
							{ maxPosLocal_Y = rVert0._pos_LocalOnMesh.y; }

							if (rVert1._pos_LocalOnMesh.x < minPosLocal_X)
							{ minPosLocal_X = rVert1._pos_LocalOnMesh.x; }
							if (rVert1._pos_LocalOnMesh.x > maxPosLocal_X)
							{ maxPosLocal_X = rVert1._pos_LocalOnMesh.x; }
							if (rVert1._pos_LocalOnMesh.y < minPosLocal_Y)
							{ minPosLocal_Y = rVert1._pos_LocalOnMesh.y; }
							if (rVert1._pos_LocalOnMesh.y > maxPosLocal_Y)
							{ maxPosLocal_Y = rVert1._pos_LocalOnMesh.y; }
						}
		...
		public static void DrawSelectedBonePost(apBone bone)
		{
			...
			if (bone._isIKAngleRange && bone._parentBone != null)
			{
				...
				Vector2 unitVector_Lower = apMatrix3x3.TRS(Vector2.zero, bone._IKAngleRange_Lower, Vector2.one).MultiplyPoint(unitVector);
				Vector2 unitVector_Upper = apMatrix3x3.TRS(Vector2.zero, bone._IKAngleRange_Upper, Vector2.one).MultiplyPoint(unitVector);
				Vector2 unitVector_Pref = apMatrix3x3.TRS(Vector2.zero, bone._IKAnglePreferred, Vector2.one).MultiplyPoint(unitVector);
