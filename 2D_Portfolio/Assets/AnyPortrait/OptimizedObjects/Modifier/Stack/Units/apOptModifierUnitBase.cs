...
namespace AnyPortrait
{
	...
	public class apOptModifierUnitBase
	{
		...
		private List<Vector2> tmpPosList = null;
		...
		public void Bake(apModifierBase srcModifier, apPortrait portrait)
		{
			...
			for (int i = 0; i < srcModifier._paramSetGroup_controller.Count; i++)
			{
				...
				optParamSetGroup.Bake(portrait, this, srcParamSetGroup);
		...
		private void Calculate_Rigging(float tDelta)
		{
			...
			//List<Vector2> tmpPosList = null;
			//subParamGroupList = null;
			//subParamKeyValueList = null;
				...
				for (int iSubList = 0; iSubList < subParamGroupList.Count; iSubList++)
				{
					...
					for (int iPV = 0; iPV < nParamKeys; iPV++)
					{
						...
						for (int iPos = 0; iPos < posList.Count; iPos++)
						{
							...
							tmpVertRig = paramKeyValue._modifiedMesh._vertRigs[iPos];

							tmpVertLocal = paramKeyValue._modifiedMesh._targetMesh.LocalVertPositions[tmpVertRig._vertIndex];
							tmpVertPosW_NoMod = tmpMatx_MeshW_NoMod.MulPoint2(tmpMatx_Vert2Local.MultiplyPoint(tmpVertLocal));

							//2. Bone의 (Mod가 적용 안된) World Matrix의 역행렬을 계산하여 Local Vert by Bone을 만든다.
							//3. Bone의 World Matrix를 계산하여 연산한다.

							tmpTotalBoneWeight = 0.0f;

							for (int iWeight = 0; iWeight < tmpVertRig._weightPairs.Count; iWeight++)
							{
								tmpWeightPair = tmpVertRig._weightPairs[iWeight];

								if (tmpWeightPair._weight <= 0.0001f)
								{
									continue;
								}

								tmpMatx_boneWorld_Mod = tmpWeightPair._bone._worldMatrix;

								tmpMatx_boneWorld_Default = tmpWeightPair._bone._worldMatrix_NonModified;

								//World -> Bone Local
								tmpVertPos_BoneLocal = tmpMatx_boneWorld_Default.InvMulPoint2(tmpVertPosW_NoMod);//<<Local Pos를 역추적

								//Bone Local -> World
								tmpVertPosW_BoneWorld = tmpMatx_boneWorld_Mod.MulPoint2(tmpVertPos_BoneLocal);

								//다시 이것의 Local Pos를 구한다.
								tmpVertPosL_Result = tmpMatx_Vert2Local.inverse.MultiplyPoint(tmpMatx_MeshW_NoMod.InvMulPoint2(tmpVertPosW_BoneWorld));

								tmpPosList[iPos] += new Vector2(tmpVertPosL_Result.x, tmpVertPosL_Result.y) * tmpWeightPair._weight;
								//tmpPosList[iPos] += tmpWeightPair._localPos * tmpWeightPair._weight;


								tmpTotalBoneWeight += tmpWeightPair._weight;

							}



							if (tmpTotalBoneWeight > 0.0f)
							{
								tmpPosList[iPos] = new Vector2(tmpPosList[iPos].x / tmpTotalBoneWeight, tmpPosList[iPos].y / tmpTotalBoneWeight);
								//+ new Vector2(UnityEngine.Random.Range(-10, 10), 0);
							}
							else
							{
								tmpPosList[iPos] = tmpVertLocal;
							}

							//if (iPos == 17)
							//{
							//	Debug.Log(">>Result Rig Pos [" + tmpPosList[iPos] + "] (" + tmpTotalBoneWeight + ")");
							//}
							...
							)
						{
							...
							for (int iPos = 0; iPos < posList.Count; iPos++)
							{
								...
								if (isValidFrame)
								{
									...
									tmpModVertWeight._velocity_Next = (tmpModVertWeight._velocity_Real * inertiaK + tmpModVertWeight._velocity_1F * (1.0f - inertiaK))
												+ (tmpF_sum / tmpMass) * tDelta;

									//if (tmpF_sum.magnitude > 10 && isFirstDebug && tmpModVertWeight._weight > 0.7f)
									//{
									//	Debug.Log("PosReal : " + tmpModVertWeight._pos_Real 
									//		+ " / Velocity Real : " + tmpModVertWeight._velocity_Real 
									//		+ " << Vel_1F : " + tmpModVertWeight._velocity_1F 
									//		+ " / tDelta_1F : " + tmpModVertWeight._tDelta_1F 
									//		+ " / F-sum : " + tmpF_sum + " / Mass : " + tmpMass
									//		+ " / Inertial (" + inertiaK + ") >> Result : " + tmpModVertWeight._velocity_Next);
									//	isFirstDebug = false;
									//	iDebugLog = iPos;
									//}

									//tmpModVertWeight._velocity_Next = (tmpModVertWeight._velocity_1F) + (tmpF_sum / tmpMass) * tDelta;
									//-------------------------------
							...
							for (int iPos = 0; iPos < posList.Count; iPos++)
							{
								...
								if (isValidFrame)
								{
									...
									//if(iPos == iDebugLog)
									//{
									//	Debug.Log("Pos 1F : " + tmpModVertWeight._pos_1F + " >> Pos Cur : " + tmpModVertWeight._pos_Real + " (Delta Pos : " + (tmpModVertWeight._pos_Real - tmpModVertWeight._pos_1F) + ")");
									//	Debug.Log("Vel 1F : " + tmpModVertWeight._velocity_1F + " >> Vel Cur " + tmpModVertWeight._velocity_Real + " >> Vel Next : " + tmpNextVelocity);
									...
									tmpModVertWeight._velocity_Next = tmpNextVelocity;
									//tmpModVertWeight._velocity_1F = tmpNextVelocity;
									tmpModVertWeight._velocity_1F = tmpModVertWeight._velocity_Real;
									...
									//tmpModVertWeight._calculatedDeltaPos_Prev = tmpModVertWeight._calculatedDeltaPos;
									//tmpModVertWeight._calculatedDeltaPos += (tmpNextVelocity) * tDelta;
