...
namespace AnyPortrait
{
	...
	public class apModifierBase
	{
		...
		public virtual void RefreshParamSet()
		{
			...
			//ParamSet이 없는 경우
			int nRemovedNoParamSet = _paramSetGroup_controller.RemoveAll(delegate (apModifierParamSetGroup a)
			{
				return a._paramSetList.Count == 0;
			});

			//이 로그는 띄우지 말자 > 삭제는 자동
			//if (nRemovedNoParamSet > 0)
			//{
			//	Debug.LogError("[" + nRemovedNoParamSet + "] 개의 ParamSet이 없는 ParamSetGroup을 삭제했습니다. (" + DisplayName + ")");
			//}

			//추가 : SyncTarget에 따라서 삭제 여부를 체크하자
			int nRemovedNoLink = _paramSetGroup_controller.RemoveAll(delegate (apModifierParamSetGroup a)
			{
													...
													bool isRecursiveAvailable = false, bool isRefreshLink = true)
		{
			...
			{
				Debug.LogError("AddBone 실패 : MeshGroup [" + (meshGroupOfBone != null) + "]");
				return null;
			}
			...
			{
				Debug.LogError("AddBone 실패 : MeshGroupTF [" + (meshGroupTransform != null) + "] <" + meshGroupOfBone._name + " : " + meshGroupOfModifier._name + ">");
				return null;
			}
			...
			if (renderUnit == null)
			{
				Debug.LogError("AddBone 실패 : Render Unit 없음");
				return null;
			}
		...
		protected void CalculatePattern_Morph(float tDelta)
		{
			...
			for (int iCalParam = 0; iCalParam < _calculatedResultParams.Count; iCalParam++)
			{
				...
				for (int iSubList = 0; iSubList < subParamGroupList.Count; iSubList++)
				{
					...
					if (tmpPosList.Count != posList.Count)
					{
						tmpPosList.Clear();
						for (int iPos = 0; iPos < posList.Count; iPos++)
						{
							tmpPosList.Add(Vector2.zero);
						}
					}
					else
					{
					...
					if (iCalculatedSubParam == 0)//<<변경
					{
						...
						//calParam.LinkedMatrix.SetPassAndMerge(apLinkedMatrix.VALUE_TYPE.VertPos);
				...
				//if(isColorUpdate)
				//{
				//	Debug.Log("CP-Morph Result Color : " + calParam._result_Color);
				//}
			}
			//if(calParam._isAvailable)
			//{
			//	//Debug.Log("Mod Cal Param Enable : " + calParam._debugID + " / " + calParam._targetRenderUnit.Name + " / " + DisplayName + "(Weight : " + calParam._totalParamSetGroupWeight + ")");
			//}
			//else
			//{
			//	Debug.LogError("Mod Cal Param Unable : " + calParam._targetRenderUnit.Name + " / " + DisplayName);
			//}
		}
...
#region [미사용 코드]
									...
									modVertWeight._velocity_Next = (modVertWeight._velocity_Real * inertiaK + modVertWeight._velocity_1F * (1.0f - inertiaK))
										+ (F_sum / mass) * tDelta;
...
#region [미사용 코드]
								...
								{
									//modVertWeight._velocity_Next = Vector2.zero;
							...
							for (int iPos = 0; iPos < posList.Count; iPos++)
							{
								...
								if (isValidFrame)
								{
									...
									//만약 이전 프레임에서 계산된 값이 없다면
									//속도와 위치값을 초기화
									//if(!modVertWeight._isPhysicsCalculatedPrevFrame)
									//{
									//	modVertWeight._calculatedDeltaPos_Prev = modVertWeight._calculatedDeltaPos;
									...
									modVertWeight._velocity_1F = nextVelocity;
