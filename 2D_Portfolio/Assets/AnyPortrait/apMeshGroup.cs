...
namespace AnyPortrait
{
	...
	public class apMeshGroup : MonoBehaviour
	{
		...
		public void UpdateRenderUnits(float tDelta, bool isUpdateVertsAlways)
		{
			//일정 프레임마다 업데이트하도록 한다.
			_tDelta += tDelta;
			//if(_tDelta > _secPerFrame)
			//{
			//_tDelta -= _secPerFrame;

			Profiler.BeginSample("MeshGroup Update Per Frame");

			//UpdateAllRenderUnits(_tDelta, isUpdateVertsAlways, false, _parentPortrait.UpdateKeyIndex);
		...
		public void RefreshForce(bool isDepthChanged = false, float tDelta = 0.0f)
		{
			...
			//UpdateAllRenderUnits(tDelta, true, isDepthChanged, _parentPortrait.UpdateKeyIndex);
		...
		///// <summary>
		///// 이 함수를 출력하면 해당 RenderUnit은 딜레이 없이 바로 갱신이 된다.
		///// Gizmo 처리시 꼭 호출해주자
		///// </summary>
		///// <param name="forceUpdateTarget"></param>
		//public void AddForceUpdateTarget(apRenderUnit forceUpdateTarget)
		//{
		//	_forceUpdateTargets.Add(forceUpdateTarget);
		//	_isAnyForceUpdate = true;
		//	_funcForceUpdate = IsForceUpdate;
		//}

		///// <summary>
		///// 모든 Render Unit을 딜레이 시간 없이 강제로 업데이트한다.
		///// 주의 : 전체 초기화나 편집 모드에 들어갈 때의 한번만 이 함수를 호출하도록 한다.
		///// </summary>
		//public void SetAllRenderUnitForceUpdate()
		//{
		//	_forceUpdateTargets.Clear();
		//	for (int i = 0; i < _renderUnits_All.Count; i++)
		//	{
		//		_forceUpdateTargets.Add(_renderUnits_All[i]);
		//	}
		//	_isAnyForceUpdate = true;
		//	_funcForceUpdate = IsForceUpdate;
		//}

		//public bool IsForceUpdate(apRenderUnit renderUnit)
		//{
		//	if(!_isAnyForceUpdate)
		//	{
		//		return false;
		//	}
		//	return _forceUpdateTargets.Contains(renderUnit);
		//}
		//public delegate bool FUNC_IS_FORCE_UPDATE(apRenderUnit renderUnit);
		...
		private void UpdateAllRenderUnits(float tDelta, bool isUpdateVertAlways, bool isDepthChanged)
		{
			...
			if (_isNeedRenderUnitReset || _renderUnits_All.Count == 0)
			{
				ResetRenderUnits();
			}
			...
			//_rootRenderUnit.Update_Pre(tDelta, (isDepthChanged || _isUpdateByBias), _funcForceUpdate);//Vertex + WorldMatrix가 계산된다.//<<이것도 Pre/Post로 나눠야 한다.
			...
			//if(_isAnyForceUpdate)
			//{
			//	_isAnyForceUpdate = false;
			//	_forceUpdateTargets.Clear();
			//	_funcForceUpdate = null;
			//}
		}
		...
		public void ResetRenderUnits()
		{
			...
			_parentPortrait.LinkAndRefreshInEditor();
		...
		public void ChangeRenderUnitDetph(apRenderUnit renderUnit, int nextDepth)
		{
			...
			if (renderUnit._depth < nextDepth)
			{ isIncrese = true; }
			else
			{ isIncrese = false; }
